...
function apply(ctx) {
	...
	/** Load the plugin-market registry: local dshmarket snapshot first, then network. */
	...
	const routes = [
		{ kind: "exact", path: `${base}/list`, handler: (req, res) => { void handleList().then((r) => writeJson(res, 200, r)); } },
