...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			console.log("[settings-nav-organizer] v1.6.16 loaded");
			// 官方设置入口（收进置顶的「DeepSeek Harness 设置」核心分组行）：
			// 通用 / 模型 / 插件 / 预设。
			const OFFICIAL = new Set(["general", "models", "plugins", "agent-presets"]);
			// 本插件的管理页（保持平铺，不进核心分组）
			const FLAT = new Set(["settings-nav-groups", "settings-nav-manage"]);
			...
			ctx.effect(() => ctx.locale.register(NS, "en", {
				...
				coreGroup: "DeepSeek Harness Settings",
			...
			const applyRules = () => {
				...
				// 核心分组（官方设置入口）显隐规则：置顶的「DeepSeek Harness 设置」行
				parts.push(`${FOLD_SEL} > button[data-snav-group="core"]{display:none}`);
				parts.push(`${FOLD_SEL}[data-snav-open-core="1"] > button[data-snav-group="core"]{display:flex}`);
			...
			// ---- 注入行：核心分组行（最前）+ 每个自定义分组一行 + 未分组「插件入口」一行 ----
			const groupRows = new Map(); // gid -> element
			let coreRow = null; // 置顶的「DeepSeek Harness 设置」核心分组行
			let pluginsRow = null;
			...
			const updateRow = (el, kind, gid) => {
				if (el === null) return;
				const isCore = kind === "group" && gid === "core";
				const group = isCore ? undefined : (kind === "group" ? state.groups.find((g) => g.id === gid) : undefined);
				const isOpen = isCore ? state.open.core === true : (kind === "group" ? state.open[gid] === true : !state.folded);
				const name = isCore ? t("coreGroup") : (kind === "group" ? (group?.name ?? "") : t("plugins"));
				const count = isCore
					? rows.filter((r) => OFFICIAL.has(r.id)).length
					: kind === "group"
			...
			const sync = () => {
				...
				try {
					...
					if (!state.foldThirdParty) {
						...
						navList.removeAttribute("data-snav-open-core");
						for (const [, el] of groupRows) el.remove();
						groupRows.clear();
						if (coreRow !== null && coreRow.parentNode !== null) coreRow.remove();
						coreRow = null;
					...
					const injected = new Set([...groupRows.values(), coreRow, pluginsRow].filter(Boolean));
					...
					// 2) 给每个原生按钮打标记：官方=核心分组 / 本插件管理页=平铺 / 有分组=组 id / 未分组=插件入口
					const coreEls = [];
					const flatEls = [];
					...
					// 核心分组展开态（data-snav-open-core）
					if (state.open.core) navList.setAttribute("data-snav-open-core", "1");
					else navList.removeAttribute("data-snav-open-core");
					...
					if (coreRow === null) coreRow = makeRow("group", "core");
					if (pluginsRow === null) pluginsRow = makeRow("plugins");
					// 5) 一次性目标序列布局（取代“行对齐 + 归位”两步，避免两者
					//    互相拆台造成往返移动死循环）：
					//    核心分组行 → 核心项 → (分组行 + 组内条目)* → 插件入口行 → 未分组条目 → 管理页
					//    条目顺序按台账顺序（基于 buttonRefs，与 DOM 顺序无关）
					const seq = [];
					if (coreRow !== null) seq.push(coreRow);
			...
			ctx.effect(() => () => {
				...
				for (const [, el] of groupRows) el.remove();
				if (coreRow !== null) coreRow.remove();
