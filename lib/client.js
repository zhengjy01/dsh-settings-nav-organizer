...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			console.log("[settings-nav-organizer] v1.5.2 loaded");
			...
			const autoClassify = async () => {
				...
				const svc = ctx.get("remote")?.settingsNavManage;
				if (svc === undefined) return { assigned: 0, offline: true };
				const fresh = rows.filter((r) => !CORE.has(r.id) && groupOf(r.id) === undefined && !autoSeen.has(r.id));
				if (fresh.length === 0) return { assigned: 0 };
				for (const r of fresh) autoSeen.add(r.id);
				let assigned = 0;
				try {
					const unknown = [];
					const res = await svc.lookupCategories({ names: fresh.map((r) => r.id) });
					if (res.ok) {
					...
					if (unknown.length > 0 && aiConfigured()) {
						const ai = await svc.classifyAI({ names: unknown, config: state.ai });
						if (ai.ok) {
			...
			function GroupsPage(props) {
				...
				const runNow = async () => {
					...
					const svc = ctx.get("remote")?.settingsNavManage;
					if (svc === undefined) {
						setAiNotice({ kind: "error", text: tPage("aiHostOffline") });
						setAiBusy(false);
						return;
					}
					try {
						...
						if (r.offline) setAiNotice({ kind: "error", text: tPage("aiHostOffline") });
						else if (r.assigned > 0) setAiNotice({ kind: "ok", text: tPage("aiAssigned").replace("{n}", String(r.assigned)) });
						else setAiNotice({ kind: "ok", text: tPage("aiEmpty") });
					} catch (e) {
				...
				const refreshModels = async () => {
					...
					try {
						const res = await svc.listModels({ config: { baseUrl: aiDraft.baseUrl.trim(), apiKey: aiDraft.apiKey.trim() } });
						if (res.ok) {
				...
				return React.createElement("div", { className: "dsh-snav-page" },
					React.createElement("div", { className: "dsh-snav-gcard" },
						...
						aiOn
							...
							: null,
						aiNotice !== null
							? React.createElement("p", { className: aiNotice.kind === "ok" ? "dsh-snav-ok" : "dsh-snav-err" }, aiNotice.text)
							: null,
					),
			...
			function ManagePage(props) {
				...
				// 渲染时动态获取宿主端管理服务（remote 命名空间须经 remote 服务对象
				// 访问：ctx.get("remote.settingsNavManage") 的点分名永远不会命中，
				// 因为命名空间由 api-remotes $mount 挂载而非注册为 cordis 服务）
