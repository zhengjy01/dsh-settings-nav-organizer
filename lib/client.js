...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			console.log("[settings-nav-organizer] v1.6.18 loaded");
			// 官方设置入口（原生平铺展开显示）：通用 / 模型 / 插件 / 预设 /
			// 插件管理（本插件注册的管理页，用户要求放前面）。
			const OFFICIAL = new Set(["general", "models", "plugins", "agent-presets", "settings-nav-manage"]);
			// 分组管理（用户明确要求放最底部）
			const FLAT = new Set(["settings-nav-groups"]);
			// 始终属于导航管理范畴的内置项（不计入插件入口折叠计数）
			const CORE = new Set([...OFFICIAL, ...FLAT]);
			...
			ctx.effect(() => ctx.locale.register(NS, "zh", {
				...
				aiTest: "测试配置",
				aiTesting: "测试中…",
				aiTestOk: "配置有效：模型响应「{reply}」",
				aiTestFail: "测试失败：{msg}",
				aiDetected: "检测到 {provider} API，已自动加载其模型列表",
				aiRunning: "分类中…",
				...
				pruneTitle: "闲置插件自动清理",
				pruneDesc: "开启后扫描各插件的工具调用记录，识别超过「闲置天数」未使用的插件，可一键清理（预览勾选后确认）。",
				pruneOn: "自动清理闲置插件",
				pruneOnDesc: "开启后，本页列出闲置插件，你可一键清理（默认超过 30 天未调用任何工具）。",
				pruneDays: "闲置天数",
				pruneSelectAll: "全选闲置插件",
				pruneSummary: "阈值 {days} 天：共 {total} 个插件，{idle} 个闲置候选；其余为在用或无工具可检测，可手动勾选。",
				pruneIdleHead: "闲置候选（可一键清理）",
				pruneRestHead: "其余插件（在用 / 无工具可检测，可手动勾选）",
				pruneNoIdle: "当前没有超过 {days} 天未使用的插件。",
				pruneScan: "扫描使用情况",
				pruneRescan: "重新扫描",
				pruneScanning: "扫描中…",
				pruneLoading: "加载中…",
				pruneEmpty: "没有插件数据。",
				pruneCalls: "调用 {c} 次",
				pruneLastUsed: "最近使用 {d}",
				pruneStateActive: "使用中",
				pruneStateIdle: "闲置 {d} 天",
				pruneStateNever: "从未检测到使用",
				pruneStateUndetectable: "无工具可检测·可手动勾选",
				pruneClean: "一键清理 ({n})",
				pruneConfirmTitle: "确认清理 {n} 个闲置插件？",
				pruneConfirmDesc: "将从依赖与 bundle 清单移除这些插件，重启 DSH 后不再加载；未勾选的插件不受影响。",
				pruneConfirm: "确认清理",
				pruneCancel: "取消",
				pruneCleaning: "清理中…",
				pruneDone: "已移除 {n} 个插件（重启后生效）",
				pruneHostOffline: "宿主端服务未就绪（请重启 DSH）",
				pruneProtectedHint: "核心 DSH、本插件及核心 UI Bundle 已自动保护，不会出现在列表。",
				search: "搜索设置",
				searchPlaceholder: "输入插件名或设置名，快速查找…",
				searchNoMatch: "没有匹配「{q}」的设置",
				searchClear: "清除搜索",
			...
			ctx.effect(() => ctx.locale.register(NS, "en", {
				...
				aiTest: "Test config",
				aiTesting: "Testing…",
				aiTestOk: "Config works: model replied “{reply}”",
				aiTestFail: "Test failed: {msg}",
				aiDetected: "Detected {provider} API — its models were loaded automatically",
				...
				pruneTitle: "Idle plugin auto-pruner",
				pruneDesc: "When on, scans each plugin's tool-call records and flags plugins unused beyond the idle threshold; you can one-click clean them after a preview.",
				pruneOn: "Auto-clean idle plugins",
				pruneOnDesc: "When on, this page lists idle plugins for one-click removal (plugins idle for more than the threshold).",
				pruneDays: "Idle days",
				pruneSelectAll: "Select all idle",
				pruneSummary: "Threshold {days}d: {total} plugins, {idle} idle candidates; the rest are in-use or not tool-detectable (check manually).",
				pruneIdleHead: "Idle candidates (one-click clean)",
				pruneRestHead: "Other plugins (in use / not tool-detectable, check manually)",
				pruneNoIdle: "No plugins idle beyond {days} days.",
				pruneScan: "Scan usage",
				pruneRescan: "Rescan",
				pruneScanning: "Scanning…",
				pruneLoading: "Loading…",
				pruneEmpty: "No plugin data.",
				pruneCalls: "{c} calls",
				pruneLastUsed: "last used {d}",
				pruneStateActive: "in use",
				pruneStateIdle: "idle {d}d",
				pruneStateNever: "never used",
				pruneStateUndetectable: "no tools·check manually",
				pruneClean: "Clean ({n})",
				pruneConfirmTitle: "Remove {n} idle plugins?",
				pruneConfirmDesc: "These plugins will be removed from dependencies and the bundle list, and won't load after a DSH restart; unchecked plugins are untouched.",
				pruneConfirm: "Confirm",
				pruneCancel: "Cancel",
				pruneCleaning: "Cleaning…",
				pruneDone: "Removed {n} plugins (take effect after restart)",
				pruneHostOffline: "Host service not ready (restart DSH)",
				pruneProtectedHint: "Core DSH, this plugin and the core UI bundle are auto-protected and never listed.",
				search: "Search settings",
				searchPlaceholder: "Type a plugin or setting name to find it…",
				searchNoMatch: "No settings match “{q}”",
			...
			const loadPersisted = () => {
				...
				try {
					...
					if (raw !== null) {
						...
						if (typeof data.pruneOn === "boolean") pruneOn = data.pruneOn;
						if (Number.isFinite(Number(data.pruneDays))) pruneDays = Math.max(1, Math.min(365, Number(data.pruneDays)));
				...
				return { groups, foldThirdParty, groupingMode, pruneOn, pruneDays, ai };
			};
			const savePersisted = () => {
				try {
					localStorage.setItem(STORAGE_KEY, JSON.stringify({
						...
						pruneOn: state.pruneOn,
						pruneDays: state.pruneDays,
						ai: state.ai,
					}));
				} catch (error) {
					console.warn("[settings-nav-organizer] localStorage write failed:", error);
				}
			};
			...
			const state = {
				...
				pruneOn: persisted.pruneOn,
				pruneDays: persisted.pruneDays,
				ai: persisted.ai,
				search: "", // 侧边栏搜索词（空=未搜索）
			};
			...
			const applyRules = () => {
				const parts = [
					...
					".dsh-snav-page{display:flex;flex-direction:column;gap:14px;box-sizing:border-box;width:100%;max-width:560px;min-width:0}",
					".dsh-snav-new{display:flex;gap:8px;align-items:center}",
					".dsh-snav-input{box-sizing:border-box;width:100%;flex:none;min-width:0;height:32px;padding:0 10px;border-radius:8px;border:1px solid var(--dsw-alias-border-l1,#444);background:var(--dsw-alias-bg-layer-1,transparent);color:var(--dsw-alias-label-primary,#eee);font-family:inherit;font-size:13px}",
					".dsh-snav-new .dsh-snav-input{flex:1;width:auto}",
					...
					".dsh-snav-select{box-sizing:border-box;height:26px;padding:0 8px;border-radius:8px;border:1px solid var(--dsw-alias-border-l1,#444);background:var(--dsw-alias-bg-layer-1,transparent);color:var(--dsw-alias-label-primary,#eee);font-family:inherit;font-size:12px}",
					...
					// 侧边栏搜索框
					".dsh-snav-search{display:flex;flex-direction:column;gap:4px;box-sizing:border-box;width:100%;padding:6px 12px;margin:0 0 4px}",
					".dsh-snav-search-row{display:flex;align-items:center;gap:6px;width:100%}",
					".dsh-snav-search-input{flex:1;min-width:0;box-sizing:border-box;height:32px;padding:0 10px;border-radius:8px;border:1px solid var(--dsw-alias-border-l1,#444);background:var(--dsw-alias-bg-layer-1,transparent);color:var(--dsw-alias-label-primary,#eee);font-family:inherit;font-size:13px}",
					".dsh-snav-search-input:focus{outline:none;border-color:var(--dsw-specific-sidebar-nav-item-hover,rgba(127,127,127,.25))}",
					".dsh-snav-search-clear{flex:none;border:none;background:transparent;color:var(--dsw-alias-label-secondary,#999);font-size:14px;cursor:pointer;padding:2px 4px}",
					".dsh-snav-search-clear:hover{color:var(--dsw-alias-label-primary,#eee)}",
					".dsh-snav-search-no{color:var(--dsw-alias-label-secondary,#999);font-size:12px;padding:2px 4px}",
					// 搜索激活：命中项强制显示（覆盖折叠/分组），未命中项隐藏
					`${FOLD_SEL}[data-snav-search="1"] > button:not([data-snav-hit="1"]){display:none}`,
					`${FOLD_SEL}[data-snav-search="1"] > button[data-snav-hit="1"]{display:flex;background:color-mix(in srgb,var(--dsw-alias-state-warn-primary,#f5a524) 16%,transparent);color:var(--dsw-alias-label-primary,#eee);font-weight:600}`,
					...
					".dsh-snav-check{flex:none;accent-color:var(--dsw-alias-state-success-primary,#46a758);width:16px;height:16px;margin:0}",
					".dsh-snav-prune-row{display:flex;align-items:center;gap:8px;padding:4px 0}",
					".dsh-snav-prune-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap}",
					".dsh-snav-prune-head{display:flex;align-items:flex-end;justify-content:space-between;gap:8px;flex-wrap:wrap}",
				];
				...
				// 侧边栏溢出修复：条目多时列表容器可滚动（避免「显示不全且无滚动条」）
				parts.push(`${FOLD_SEL}{overflow-y:auto;min-height:0}`);
				parts.push(`[role="dialog"][aria-modal="true"] > nav{overflow-y:auto;min-height:0}`);
			...
			let pluginsRow = null;
			let searchBox = null; // 侧边栏搜索框（含 clear / 无匹配提示）

			// ---- 独立搜索刷新观察器（定义提前，供 applySearch 调用）----
			// 搜索激活时，导航列表的 DOM 变化（React 重建按钮）可能发生在主观察器风暴挂起期间，
			// 导致 sync → applySearch 不执行、命中标记丢失 → 搜索"失效"。这里用一个专注的观察器
			// 只监听 navList，一旦其子树变化就重跑 applySearch（仅搜索激活时活跃，不常驻，不会引发风暴）。
			let searchObs;
			let searchObsTarget = null;
			const ensureSearchObs = (target) => {
				if (searchObs === undefined) searchObs = new MutationObserver(() => {
					if (state.search !== "") {
						// 节流：下个宏任务再跑，避免 observer 自身的 DOM 写入触发自身
						setTimeout(() => { if (state.search !== "") applySearch(); }, 0);
					}
				});
				if (target !== null && searchObsTarget !== target) {
					searchObs.disconnect();
					searchObs.observe(target, { childList: true, subtree: true });
					searchObsTarget = target;
				}
			};
			const stopSearchObs = () => {
				if (searchObs !== undefined && searchObs !== null) searchObs.disconnect();
				searchObsTarget = null;
			};

			// 构建搜索框：输入即按名称/ID 模糊过滤所有导航条目。
			// 用 navList 上的 data-snav-search="1" 激活 CSS 过滤规则，搜索结束移除即恢复原折叠/分组状态。
			const makeSearch = () => {
				const wrap = document.createElement("div");
				wrap.className = "dsh-snav-search";
				wrap.dataset.snavRow = "search";
				const row = document.createElement("div");
				row.className = "dsh-snav-search-row";
				const input = document.createElement("input");
				input.type = "text";
				input.className = "dsh-snav-search-input";
				input.placeholder = t("searchPlaceholder");
				input.setAttribute("aria-label", t("search"));
				const clear = document.createElement("button");
				clear.type = "button";
				clear.className = "dsh-snav-search-clear";
				clear.textContent = "✕";
				clear.title = t("searchClear");
				const no = document.createElement("span");
				no.className = "dsh-snav-search-no";
			...
			// 名称/ID 模糊匹配；favor label，其次 id
			const searchMatches = (id, label) => {
			...
			// 应用搜索过滤：命中项标 data-snav-hit="1"（CSS 强制显示），未命中不标（CSS 隐藏）。
			// 分组行在组内有命中时显示；插件入口行在未分组有命中时显示；官方入口同样参与。
			//
			// 注意：不能用 buttonRefs 直接遍历——React 重建导航按钮后 buttonRefs 可能引用
			// 已被移除的旧元素（refresh 只在台账变化时触发，重建不更新引用表），会导致新按钮
			// 无命中标记、全部被 CSS 隐藏 → 搜索无效。这里每次基于当前 navList 的 DOM 按钮逐个
			// 按 label 匹配台账条目来打标记，天然跟 DOM 同步。
			const applySearch = () => {
				const navList = findNavList();
				if (navList === null) return;
				const on = state.search !== "";
				if (on) navList.dataset.snavSearch = "1";
				else {
					delete navList.dataset.snavSearch;
					stopSearchObs(); // 搜索结束：停掉刷新观察器
				}
				// DOM 变化时自动重跑搜索（React 重建按钮后命中不过期）
				if (on) ensureSearchObs(navList);
				// 清掉全部命中标记（含当前 DOM 里所有按钮，避免重建残留）
				for (const child of navList.children) {
					if (child.tagName !== "BUTTON") continue;
					if (child.dataset.snavHit !== undefined) delete child.dataset.snavHit;
				}
				if (pluginsRow !== null && pluginsRow.dataset.snavHit !== undefined) delete pluginsRow.dataset.snavHit;
				// 更新 noMatch 提示
				if (searchBox !== null) {
					const no = searchBox.querySelector(".dsh-snav-search-no");
					if (no !== null) {
						const anyHit = [...navList.children].some((c) => c.tagName === "BUTTON" && c.dataset.snavHit === "1");
						no.style.display = on && !anyHit ? "" : "none";
						if (on && !anyHit) no.textContent = t("searchNoMatch").replace("{q}", state.search);
					}
				}
				if (!on) return; // 未搜索：交还给 CSS 折叠/分组规则
				const hitIds = new Map(rows.map((r) => [r.id, r]));
				const matchBtn = (id) => {
					const r = hitIds.get(id);
					return searchMatches(id, r?.label);
				};
				// 官方入口 + 未分组条目：按当前 DOM 按钮匹配（label 匹配台账）
				for (const child of navList.children) {
					if (child.tagName !== "BUTTON") continue;
					// 跳过注入行（分组行/插件入口行由下方单独处理）
					if (child.classList.contains("dsh-snav-chip")) continue;
					// 用按钮文本反查台账条目（r.label 是唯一显示文本）
					const text = (child.textContent ?? "").trim();
					const row = rows.find((r) => r.label === text);
					if (row !== undefined && matchBtn(row.id)) child.dataset.snavHit = "1";
				}
				// 分组行：组内有命中则显示（内部命中条目已由上面标记）
				for (const g of state.groups) {
					const row = groupRows.get(g.id);
					if (row === undefined) continue;
					const ghit = g.entries.some((id) => matchBtn(id));
					if (ghit) row.dataset.snavHit = "1";
				}
				// 插件入口行：未分组有命中则显示
				if (pluginsRow !== null) {
					const uhit = rows.some((r) => !CORE.has(r.id) && groupOf(r.id) === undefined && matchBtn(r.id));
					if (uhit) pluginsRow.dataset.snavHit = "1";
				}
			};
			...
			const makeRow = (kind, gid) => {
				...
				el.addEventListener("click", () => {
					if (kind === "group") {
						...
						// 即时更新显隐属性：即使此刻 sync 被观察器占用（inSync），
						// 折叠/展开也立即生效，避免「展开后收不回（固定死）」
						const list = findNavList();
						if (list !== null) {
							if (state.open[gid]) list.setAttribute(`data-snav-open-${gid}`, "1");
							else list.removeAttribute(`data-snav-open-${gid}`);
						}
					} else {
						state.folded = !state.folded;
						const list = findNavList();
						if (list !== null) {
							if (state.folded) list.setAttribute("data-snav-folded", "1");
							else list.removeAttribute("data-snav-folded");
						}
					}
					requestSync();
				});
			...
			// 但用户点击等关键交互若撞上 inSync，必须排队重跑，不能吞掉状态变化。
			let inSync = false;
			...
			const sync = () => {
				...
				try {
					const navList = findNavList();
					if (navList === null) return;
					// 折叠总开关关闭：恢复原生导航——清除标记、移除插件/分组注入行；搜索框保留
					if (!state.foldThirdParty) {
						...
						// 搜索框仍应显示在顶部
						if (searchBox === null) searchBox = makeSearch();
						if (searchBox.parentNode !== navList) navList.insertBefore(searchBox, navList.firstChild);
						applySearch();
						return;
					}
					...
					if (searchBox === null) searchBox = makeSearch();
					const injected = new Set([searchBox, ...groupRows.values(), pluginsRow].filter(Boolean));
					// 1) 同步按钮引用表：清理失效引用，把新按钮（未被引用的按钮，
					//    例如 React 重建）匹配到尚未被引用的台账条目。
					//    匹配策略：优先按 label 文本（DOM 顺序与台账 order 可能不一致，
					//    顺序匹配会整体错位 → 按钮被标记成别的条目、插件入口错乱）；
					//    文本匹配不到的（重复/空 label）再按 DOM 顺序兜底。
					const alive = new Set();
					for (const child of navList.children) {
					...
					const remainingRows = rows.filter((r) => !buttonRefs.has(r.id));
					// 第一轮：label 文本匹配
					for (const child of [...navList.children]) {
						if (injected.has(child) || child.tagName !== "BUTTON") continue;
						if (taken.has(child)) continue;
						const text = (child.textContent ?? "").trim();
						const idx = remainingRows.findIndex((r) => (r.label ?? "").trim() === text && text !== "");
						if (idx >= 0) {
							const row = remainingRows[idx];
							buttonRefs.set(row.id, child);
							remainingRows.splice(idx, 1);
							taken.add(child);
						}
					}
					// 第二轮：剩余按钮按 DOM 顺序兜底匹配剩余台账
					let bi = 0;
					for (const child of [...navList.children]) {
						if (injected.has(child) || child.tagName !== "BUTTON") continue;
						if (taken.has(child)) continue;
						while (bi < remainingRows.length && buttonRefs.has(remainingRows[bi].id)) bi += 1;
						if (bi < remainingRows.length) {
							buttonRefs.set(remainingRows[bi].id, child);
							taken.add(child);
						}
					}
					// 2) 给每个原生按钮打标记：官方入口=平铺 / 分组管理=平铺末尾 / 有分组=组 id / 未分组=插件入口
					const flatEls = [];
					...
					//    互相拆台造成往返移动死循环）：
					//    搜索框(固定最前，仅作锚点) → 核心项(官方入口平铺，按台账 order，含插件管理)
					//    → 分组管理 → (分组行 + 组内条目)* → 插件入口行 → 未分组条目
					//    条目顺序按台账顺序（基于 buttonRefs，与 DOM 顺序无关）
					//
					//    注意：searchBox 不进入 seq（否则每次 sync 会与官方入口往返移动触发
					//    mutation storm），而是先确保它在正确位置，再作为 seq 布局的锚点。
					// 确保 searchBox 在 navList 的最前（作为锚点，anchor 不移动）
					if (searchBox !== null && searchBox.parentNode !== navList) {
						navList.insertBefore(searchBox, navList.firstChild);
					} else if (navList.firstChild !== searchBox) {
						navList.insertBefore(searchBox, navList.firstChild);
					}
					let seqAnchor = searchBox; // 锚点=搜索框（其顺序不动）
					const seq = [];
					for (const r of rows) {
						if (!OFFICIAL.has(r.id)) continue;
						const btn = buttonRefs.get(r.id);
						if (btn !== undefined) seq.push(btn);
					}
					// 分组管理：原生入口区的最末尾（紧跟插件管理）、折叠分组区的最前面
					for (const el of flatEls) seq.push(el);
					for (const g of state.groups) {
						const row = groupRows.get(g.id);
						if (row === undefined) continue;
						seq.push(row);
			...
			// 预设供应商 → 默认 API 地址（选择预设模型时自动填入 URL 输入框）
			const PROVIDER_URLS = {
			...
			// URL → 服务商推断（用于自动匹配预设模型组并置顶）
			const detectProvider = (url) => {
				const u = (url ?? "").toLowerCase();
				if (u.includes("deepseek")) return "DeepSeek";
				if (u.includes("openai")) return "OpenAI";
				if (u.includes("dashscope") || u.includes("aliyun")) return "通义千问 Qwen";
				if (u.includes("bigmodel") || u.includes("zhipu")) return "智谱 GLM";
				if (u.includes("moonshot") || u.includes("kimi")) return "Kimi";
				if (u.includes("doubao") || u.includes("volces")) return "豆包 Doubao";
				return null;
			};
			// 名称启发式分类（兜底：市场查不到、未配置 AI 时按插件名猜分类）
			const NAME_RULES = [
				{ re: /(sidebar|side-?bar|侧栏|侧边栏|navigation|navbar|面板)/i, cat: "ui" },
				{ re: /(theme|skin|皮肤|壁纸|wallpaper|appearance)/i, cat: "theme" },
				{ re: /(provider|llm|模型|接入|deepseek|openai|qwen|glm|kimi|doubao|claude|gemini|moonshot)/i, cat: "models" },
				{ re: /(memory|记忆|rememb|note|笔记|flomo|notion|知识)/i, cat: "memory" },
				{ re: /(notif|通知|alert|提醒|bark|telegram|feishu|wechat|dingtalk|slack|discord|push)/i, cat: "notifications" },
				{ re: /(skill|技能)/i, cat: "skills" },
				{ re: /(workflow|工作流|pipeline)/i, cat: "workflow" },
				{ re: /(game|gomoku|棋|娱乐|pet|宠物|fun|music|音乐)/i, cat: "fun" },
				{ re: /(market|marketplace|商店|market$)/i, cat: "market" },
				{ re: /(ssh|bash|terminal|tool|工具|git|docker|npm|package|dev|开发|runtime|github|code|build|web-?ui)/i, cat: "development" },
				{ re: /(task|todo|tick|滴答|plan|待办|schedule|calendar|日历|agenda)/i, cat: "tools" },
				{ re: /(chat|message|会话|消息|conversation|voice|语音|speech|reply)/i, cat: "sessions" },
				{ re: /(ui|web|frontend|界面|视觉|vision|widget|dock|layout|搜索|search|file|文件|cloud|网盘|sync|同步)/i, cat: "ui" },
			];
			const classifyByName = (name) => {
				const n = String(name ?? "").toLowerCase();
				for (const rule of NAME_RULES) {
					if (rule.re.test(n)) return rule.cat;
				}
				return null;
			};
			const localeId = () => {
			...
			const autoClassify = async () => {
				...
				try {
					...
					// 名称启发式兜底（市场未收录时按插件名猜分类）
					const stillUnknown = [];
					for (const id of unknown) {
						const gname = groupNameOfCategory(classifyByName(id));
						if (gname !== null) {
							assignToGroup(id, gname);
							assigned += 1;
						} else {
					...
					// AI 兜底（配置了才走）
					if (stillUnknown.length > 0 && aiConfigured()) {
						const ai = await api(`${API_BASE}/classifyAI`, { names: stillUnknown, config: state.ai });
						if (ai.ok) {
			...
			function GroupsPage(props) {
				...
				// 根据 AI URL + Key 自动拉取模型列表（防抖，失败静默）
				React.useEffect(() => {
					const url = aiDraft.baseUrl.trim();
					const key = aiDraft.apiKey.trim();
					if (url === "" || key === "") return;
				...
				}, [aiDraft.baseUrl, aiDraft.apiKey]);
				// AI 配置实时自动保存：输入/选模型即持久化（防抖），
				// 无需点「保存配置」；重开设置面板时始终显示已保存的值
				React.useEffect(() => {
					const timer = setTimeout(() => {
					...
					return () => clearTimeout(timer);
					// eslint-disable-next-line react-hooks/exhaustive-deps
				}, [aiDraft.baseUrl, aiDraft.apiKey, aiDraft.model]);
				React.useEffect(() => {
				...
				const refreshModels = async (silent) => {
					...
					try {
						...
						if (res.ok) {
							...
							if (!silent) setAiNotice({ kind: "ok", text: res.value.models.length > 0 ? tPage("aiModelsFetched").replace("{n}", String(res.value.models.length)) : tPage("aiModelsEmpty") });
						} else if (!silent) {
							setAiNotice({ kind: "error", text: `${res.error?.code}: ${res.error?.message}` });
						}
					} catch (e) {
						if (!silent) setAiNotice({ kind: "error", text: String(e) });
					}
				...
				const testConfig = async () => {
					...
					setAiNotice(null);
					try {
						const res = await api(`${API_BASE}/testConfig`, {
							config: { baseUrl: aiDraft.baseUrl.trim(), apiKey: aiDraft.apiKey.trim(), model: aiDraft.model.trim() },
						});
						if (res.ok) {
							setAiNotice({ kind: "ok", text: tPage("aiTestOk").replace("{reply}", String(res.value.reply ?? "")) });
						} else {
							const detail = res.error?.status === 401
								? "401：API Key 无效或未授权"
								: res.error?.status === 404
									? "404：模型不存在或 API 路径有误"
									: `${res.error?.code ?? ""} ${res.error?.message ?? ""}`.trim();
							setAiNotice({ kind: "error", text: tPage("aiTestFail").replace("{msg}", detail) });
						}
					} catch (e) {
						setAiNotice({ kind: "error", text: tPage("aiTestFail").replace("{msg}", String(e)) });
					}
					setTestingConfig(false);
				};
				// 下拉选项：预设分组（检测到的服务商置顶）+ 远程模型 + 已保存模型兜底
				const detectedProvider = detectProvider(aiDraft.baseUrl);
				const presetGroups = [...PRESET_MODELS];
				if (detectedProvider !== null) {
					const idx = presetGroups.findIndex((pg) => pg.group === detectedProvider);
					if (idx > 0) presetGroups.splice(0, 0, presetGroups.splice(idx, 1)[0]);
				}
				const modelGroups = [];
				for (const pg of presetGroups) modelGroups.push({ label: pg.group, items: pg.models });
				...
				return React.createElement("div", { className: "dsh-snav-page" },
					React.createElement(FoldToggleRow, { t: tPage }),
						...
						aiOn
							? React.createElement(React.Fragment, null,
								React.createElement("div", { className: "dsh-snav-gitems" },
									// 交互顺序：先选模型（自动带上供应商 URL）→ 填 API Key → API 地址确认
									React.createElement("select", {
										...
										onChange: (e) => {
											...
											const next = { ...aiDraft, model };
											// 选择预设模型 → 自动填入对应供应商的 API 地址
											if (model !== "") {
												for (const pg of PRESET_MODELS) {
													if (pg.models.includes(model)) {
														next.baseUrl = PROVIDER_URLS[pg.group] ?? next.baseUrl;
														break;
													}
												}
											}
									...
									detectedProvider !== null
										? React.createElement("span", { className: "dsh-snav-ok" }, tPage("aiDetected").replace("{provider}", detectedProvider))
										: null,
									React.createElement("div", { className: "dsh-snav-ghead" },
										...
										React.createElement("button", { className: "dsh-snav-btn", onClick: testConfig, disabled: testingConfig }, testingConfig ? tPage("aiTesting") : tPage("aiTest")),
									),
									React.createElement("div", { className: "dsh-snav-ghead" },
					...
					React.createElement(PruneCard, { t: tPage }),
			...
			// ---- 折叠总开关（渲染于「分组管理」页顶部，状态与 localStorage 共享）----
			...
			// ---- 闲置插件自动清理（开关 + 阈值 + 扫描 + 预览勾选 + 一键清理）----
			function PruneCard(props) {
				const tPage = props.t ?? ((k) => k);
				const [, setTick] = React.useState(0);
				const force = () => setTick((v) => v + 1);
				React.useEffect(() => {
				...
				const [plugins, setPlugins] = React.useState(null); // null = 未加载
				const [scanning, setScanning] = React.useState(false);
				const [cleaning, setCleaning] = React.useState(false);
				const [confirming, setConfirming] = React.useState(false);
				const [notice, setNotice] = React.useState(null);
				const [sel, setSel] = React.useState(new Set());
				const [days, setDays] = React.useState(state.pruneDays);
				const on = state.pruneOn;

				// 阈值变化防抖持久化
				React.useEffect(() => {
					const timer = setTimeout(() => {
						state.pruneDays = Math.max(1, Math.min(365, Number(days) || 30));
						savePersisted();
					}, 400);
					return () => clearTimeout(timer);
				}, [days]);
				...
				const scan = () => {
					setScanning(true);
					setNotice(null);
					api(`${API_BASE}/usage`, { days: Number(days) || 30 })
						.then((res) => {
							if (res.ok) {
								const list = res.value.plugins ?? [];
								setPlugins(list);
								setSel(new Set(list.filter((p) => p.recommend === true).map((p) => p.module)));
							} else {
								setPlugins([]);
								setNotice({ kind: "error", text: `${res.error?.code}: ${res.error?.message}` });
							}
						})
						.catch((e) => {
							setPlugins([]);
				...
				// 开关打开且尚未加载时立即扫描一次
				React.useEffect(() => {
					if (on && plugins === null) scan();
					// eslint-disable-next-line react-hooks/exhaustive-deps
				}, [on]);

				const toggleOn = () => {
					state.pruneOn = !state.pruneOn;
					savePersisted();
					notifyPage();
					if (state.pruneOn && plugins === null) scan();
				};
				const toggleSel = (m) => {
					setSel((prev) => {
				...
				const toggleAll = (checked) => {
					const candidates = plugins.filter((p) => p.idle === true).map((p) => p.module);
					setSel(new Set(checked ? candidates : []));
				};
				const doClean = () => {
					const names = [...sel];
					setCleaning(true);
					setNotice(null);
					api(`${API_BASE}/prune`, { names })
						.then((res) => {
							if (res.ok) {
								const done = res.value.results.filter((r) => r.ok).length;
								setNotice({ kind: "ok", text: tPage("pruneDone").replace("{n}", String(done)) });
								setConfirming(false);
								scan();
							} else {
								setNotice({ kind: "error", text: `${res.error?.code}: ${res.error?.message}` });
				...
				const selCount = plugins === null ? 0 : [...sel].filter((m) => plugins.some((x) => x.module === m)).length;
				const idleCount = plugins === null ? 0 : plugins.filter((p) => p.idle === true).length;

				const stateText = (p) => {
					if (p.reason === "active") return tPage("pruneStateActive");
					if (p.reason === "never-used") return tPage("pruneStateNever");
					if (p.reason === "undetectable") return tPage("pruneStateUndetectable");
					return tPage("pruneStateIdle").replace("{d}", String(p.daysIdle ?? 0));
				};
				const row = (p) => React.createElement("div", { key: p.module, className: "dsh-snav-prune-row" },
					React.createElement("input", {
						type: "checkbox",
						className: "dsh-snav-check",
						checked: sel.has(p.module),
						onChange: () => toggleSel(p.module),
					}),
					React.createElement("span", { className: "dsh-snav-itemname" }, p.module),
					React.createElement("span", {
						className: "dsh-snav-state",
						"data-enabled": p.idle ? "false" : "true",
					},
						stateText(p),
						!p.idle && p.reason === "active" ? ` · ${tPage("pruneLastUsed").replace("{d}", (p.daysIdle ?? 0) + "d")}` : "",
					),
					p.tracked ? React.createElement("span", { className: "dsh-snav-hint" }, tPage("pruneCalls").replace("{c}", String(p.calls))) : null,
				);
				const idleList = plugins === null ? [] : plugins.filter((p) => p.idle === true);
				const restList = plugins === null ? [] : plugins.filter((p) => p.idle !== true);

				return React.createElement("div", { className: "dsh-snav-gcard" },
					React.createElement("div", { className: "dsh-snav-ghead" },
						React.createElement("span", { className: "dsh-snav-gname" }, tPage("pruneTitle")),
						React.createElement("button", {
							type: "button",
							role: "switch",
							"aria-checked": on,
					...
					React.createElement("span", { className: "dsh-snav-hint" }, tPage("pruneDesc")),
					React.createElement("div", { className: "dsh-snav-prune-head" },
						React.createElement("div", { className: "dsh-snav-prune-actions" },
							React.createElement("label", { className: "dsh-snav-hint", style: { display: "flex", alignItems: "center", gap: 6, margin: 0 } },
								tPage("pruneDays"),
								React.createElement("input", {
									type: "number",
									className: "dsh-snav-input",
							...
							React.createElement("button", { className: "dsh-snav-btn", onClick: scan, disabled: scanning },
								scanning ? tPage("pruneScanning") : (plugins === null ? tPage("pruneScan") : tPage("pruneRescan")),
							),
						),
						plugins !== null && plugins.length > 0
							? React.createElement("button", {
								className: "dsh-snav-btn dsh-snav-danger",
								disabled: cleaning || selCount === 0,
								onClick: () => setConfirming(true),
							}, cleaning ? tPage("pruneCleaning") : tPage("pruneClean").replace("{n}", String(selCount)))
							: null,
					),
					notice !== null
						? React.createElement("p", { className: notice.kind === "ok" ? "dsh-snav-ok" : "dsh-snav-err" }, notice.text)
						: null,
					React.createElement("span", { className: "dsh-snav-hint" }, tPage("pruneProtectedHint")),
					plugins === null
						? React.createElement("p", { className: "dsh-snav-hint" }, on ? tPage("pruneLoading") : tPage("pruneEmpty"))
						: plugins.length === 0
							? React.createElement("p", { className: "dsh-snav-hint" }, tPage("pruneEmpty"))
							: React.createElement(React.Fragment, null,
								React.createElement("p", { className: "dsh-snav-hint" },
									tPage("pruneSummary")
										.replace("{days}", String(days))
										.replace("{total}", String(plugins.length))
										.replace("{idle}", String(idleCount)),
								),
								idleList.length === 0
									? React.createElement("p", { className: "dsh-snav-hint" }, tPage("pruneNoIdle").replace("{days}", String(days)))
									: React.createElement(React.Fragment, null,
										React.createElement("div", { className: "dsh-snav-ghead", style: { paddingTop: 4 } },
											React.createElement("label", { className: "dsh-snav-hint", style: { display: "flex", alignItems: "center", gap: 6, margin: 0 } },
												React.createElement("input", { type: "checkbox", className: "dsh-snav-check", checked: selCount > 0 && selCount === idleCount, onChange: (e) => toggleAll(e.target.checked) }),
												`${tPage("pruneSelectAll")} · ${idleCount}`,
											),
										),
										React.createElement("span", { className: "dsh-snav-hint" }, tPage("pruneIdleHead")),
										React.createElement("div", { className: "dsh-snav-gitems" }, idleList.map(row)),
									),
								restList.length === 0
									? null
									: React.createElement(React.Fragment, null,
										React.createElement("span", { className: "dsh-snav-hint" }, tPage("pruneRestHead")),
										React.createElement("div", { className: "dsh-snav-gitems" }, restList.map(row)),
									),
							),
					confirming
						? React.createElement(React.Fragment, null,
							React.createElement("p", { className: "dsh-snav-warn" }, tPage("pruneConfirmTitle").replace("{n}", String(selCount))),
							React.createElement("p", { className: "dsh-snav-hint" }, tPage("pruneConfirmDesc")),
							React.createElement("div", { className: "dsh-snav-prune-actions" },
								React.createElement("button", { className: "dsh-snav-btn dsh-snav-danger", onClick: doClean, disabled: cleaning }, cleaning ? tPage("pruneCleaning") : tPage("pruneConfirm")),
								React.createElement("button", { className: "dsh-snav-btn", onClick: () => setConfirming(false) }, tPage("pruneCancel")),
							),
						)
						: null,
				);
			...
			ctx.slots.inject("settings.section", () => ctx.slots.register(
				{ name: "settings.section", id: "settings-nav-groups", order: 999, label: () => t("groups"), locale: NS },
				GroupsPage,
			));
			ctx.slots.inject("settings.section", () => ctx.slots.register(
			...
			const observer = new MutationObserver((mutations) => {
				...
				for (const m of mutations) {
					...
					// 触发条件：
					//  - body 直接子级变化（面板挂载/卸载，测试环境）
					//  - 变化发生在设置 dialog 内（真实环境面板渲染在 app 容器内，
					//    target 不是 body —— 这是折叠行在打开设置时消失的根因）
					//  - 新增/移除的节点是设置 dialog（面板打开/关闭的那一次插入）
					const inDialog = target.closest?.('[role="dialog"][aria-modal="true"]') !== null;
					const hasDialog = [...(m.addedNodes ?? []), ...(m.removedNodes ?? [])].some(
						(n) => n instanceof Element && n.matches?.('[role="dialog"][aria-modal="true"]'),
					);
					if (target === document.body || inDialog || hasDialog || target.closest?.(FOLD_SEL) !== null) {
						needSync = true;
						break;
					}
				}
			...
			ctx.effect(() => () => {
				...
				if (searchBox !== null) searchBox.remove();
